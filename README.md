...
![demo](./images/video-twitter.gif)

## Stack

- Angular 22 (standalone components, zoneless: sin zone.js)
- Signals para la variante reactiva y template-driven forms (`ngModel`) para la variante clásica
- TypeScript 6, pnpm, vitest como runner de tests con cobertura, ESLint 10 + Prettier
- Typeface: Manrope a través de Google Fonts

## Cómo levantar el proyecto

```bash
pnpm install
pnpm start        # arranca en http://localhost:4200
```
...
| `pnpm test:prod`    | Tests con reporte de cobertura                |
| `pnpm lint`         | ESLint                                        |
| `pnpm lint:fix`     | ESLint con autocorrección                     |

## Las dos variantes

El ejemplo tiene dos implementaciones equivalentes que se pueden alternar desde la barra de navegación:

| URL                         | Variante                                                              |
| --------------------------- | --------------------------------------------------------------------- |
| `http://localhost:4200` o `/binding` | [Variante con template](#variante-con-template)         |
| `http://localhost:4200/signals`      | [Variante con signals](#variante-con-signals)          |
...
donde tweet es un objeto de la clase Tweet, cuando nosotros disparamos un cambio del signal `texto` Angular no recalcula el valor para el computed `cantidadLetrasRestantes`. En cambio eso sí pasa si hacemos
